...
def add_weekly_rotators(weekly_rotator_hash_list, config, logger):
    ...
    for activity_hash in weekly_rotator_hash_list:
        ...
        destiny_activity_definition_row = fetch_one(query=f"SELECT * FROM DestinyActivityDefinition Where Id = {activity_id}", config=config)
        ...
        destiny_activity_type_definition_row = fetch_one(query=f'SELECT * FROM DestinyActivityTypeDefinition Where Id = {activity_type_id}',config=config)
        ...
        if len(inventory_item_id_list) > 0:
            ...
            for item_id in inventory_item_id_list:
                ...
                if len(inventory_item_intrinsic_trait_list) > 0 and inventory_item_type != "Catalyst":
                    destiny_inventory_item_definition_intrinsic_trait_row = fetch_one(query=f"SELECT * FROM DestinyInventoryItemDefinition WHERE Id = {inventory_item_intrinsic_trait_list[0]}", config=config)
